...
def main(args):
    ...
    dataset_train = PairedGUAVADataset(dataset_path=args.dataset_path, split="train", tokenizer=net_difix.tokenizer)
    dl_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.train_batch_size, shuffle=True, num_workers=args.dataloader_num_workers)
    dataset_val = PairedGUAVADataset(dataset_path=args.dataset_path, split="test", tokenizer=net_difix.tokenizer)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.viz_freq == 1:
                        ...
                        log_dict = {
                            "[train] src/tgt/pred": [wandb.Image(
                                torch.concat([
                    ...
                    if args.eval_freq > 0 and global_step % args.eval_freq == 1:
                        ...
                        for step, batch_val in enumerate(dl_val):
                            ...
                            with torch.no_grad():
                                ...
                                if step % 10 == 0:
                                    log_dict["[sample] src/tgt/pred"].append(wandb.Image(
                                        torch.concat([
                                        ...
                                        ], dim=-1), caption=f"idx={len(log_dict['[sample] src/tgt/pred'])}"
                                    ))
