...
from model_denoise import DenoiseTurbo, load_ckpt_from_state_dict, save_ckpt
...
def main(args):
    ...
    net_denoise = DenoiseTurbo(
        lora_rank_vae=args.lora_rank_vae, 
        timestep=args.timestep,
        mv_unet=args.mv_unet,
    )
    net_denoise.set_train()

    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            net_denoise.unet.enable_xformers_memory_efficient_attention()
        else:
            raise ValueError("xformers is not available, please install it by running `pip install xformers`")

    if args.gradient_checkpointing:
        net_denoise.unet.enable_gradient_checkpointing()
    ...
    # make the optimizer
    layers_to_opt = []
    layers_to_opt += list(net_denoise.unet.parameters())
   
    for n, _p in net_denoise.vae.named_parameters():
        if "lora" in n and "vae_skip" in n:
            assert _p.requires_grad
            layers_to_opt.append(_p)
    layers_to_opt = layers_to_opt + list(net_denoise.vae.decoder.skip_conv_1.parameters()) + \
        list(net_denoise.vae.decoder.skip_conv_2.parameters()) + \
        list(net_denoise.vae.decoder.skip_conv_3.parameters()) + \
        list(net_denoise.vae.decoder.skip_conv_4.parameters())
    ...
    if args.resume is not None:
        if os.path.isdir(args.resume):
            ...
            net_denoise, optimizer = load_ckpt_from_state_dict(
                net_denoise, optimizer, ckpt_files[-1]
            )
            ...
            net_denoise, optimizer = load_ckpt_from_state_dict(
                net_denoise, optimizer, args.resume
            )    
        else:
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                # forward pass
                x_tgt_pred = net_denoise(x_src, prompt_tokens=batch["input_ids"])       
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        # accelerator.unwrap_model(net_denoise).save_model(outf)
                        save_ckpt(accelerator.unwrap_model(net_denoise), optimizer, outf)
                    ...
                    if args.eval_freq > 0 and global_step % args.eval_freq == 0:
                        ...
                        for step, batch_val in enumerate(dl_val):
                            ...
                            with torch.no_grad():
                                # forward pass
                                x_tgt_pred = accelerator.unwrap_model(net_denoise)(x_src, prompt_tokens=batch_val["input_ids"].cuda())
